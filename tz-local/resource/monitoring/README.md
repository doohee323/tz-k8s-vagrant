...
sl-eks-main/sl-local/resource/monitoring/prometheus/prometheus-values.yaml
      - job_name: 'sl-blackbox-exporter'
        metrics_path: /probe
        params:
          module: [http_2xx]
        static_configs:
          - targets:
            - http://sl-sample-app.sl-production.svc
            - http://sl-sample-app.sl-development.svc
```
...
```
메시지를 받을 수신자 지정 
sl-eks-main/sl-local/resource/monitoring/prometheus/alertmanager.values
  ...
  route:
    ...
    - receiver: 'dev_mail'
      match:
        instance: http://sl-sample-app.sl-development.svc
    - receiver: 'prod_mail'
      match:
        instance: http://sl-sample-app.sl-production.svc
    - receiver: 'dev_mail'
      match:
        namespace: 'sl-development'
  receivers:
  - name: 'k8s-admin'
    email_configs:
    - to: doohee@seerslab.com
  - name: 'dev_mail'
    email_configs:
...
## 룰 적용 (DevOps가 수행)
```
bash /vagrant/sl-local/resource/monitoring/prometheus/update.sh
