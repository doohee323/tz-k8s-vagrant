...
docker build -t harbor.harbor.topzone-k8s.drillquiz.com/topzone-k8s/kaniko-executor:v1.7.0-debug .
docker push harbor.harbor.topzone-k8s.drillquiz.com/topzone-k8s/kaniko-executor:v1.7.0-debug

/kaniko/executor --dockerfile=Dockerfile --context=/root/shared-data/tz-demo-app --build-arg NODE_ENV=development \
--destination=harbor.harbor.topzone-k8s.drillquiz.com/topzone-k8s/tz-demo-app:13 \
--use-new-run --cleanup --force --skip-tls-verify


/kaniko/executor --dockerfile=Dockerfile --context=/root/shared-data/tz-demo-app --build-arg NODE_ENV=development \
--destination=harbor.harbor.topzone-k8s.drillquiz.com/topzone-k8s/kaniko-executor:v1.7.0-debug \
--use-new-run --cleanup --force --skip-tls-verify
...
kubectl create secret docker-registry regsecret -n jenkins \
    --docker-server=harbor.harbor.topzone-k8s.drillquiz.com \
