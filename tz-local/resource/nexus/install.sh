...
echo '
##[ Nexus ]##########################################################
- url: http://192.168.0.127:8081
- id: admin
- passwd: cat /opt/sonatype-work/nexus3/admin.password

http://192.168.0.127:8081/#admin/repository/blobstores

Create blob store
  docker-hosted
  docker-hub

http://192.168.0.127:8081/#admin/repository/repositories
...
http://192.168.0.127:8081/#admin/security/realms
  add "Docker Bearer Token Realm" Active

docker login 192.168.0.127:5000
docker pull busybox
RMI=`docker images -a | grep busybox | awk '{print $3}'`
docker tag $RMI 192.168.0.127:5000/busybox:v20201225
docker push 192.168.0.127:5000/busybox:v20201225

http://192.168.0.127:8081/#browse/browse:docker-hosted
