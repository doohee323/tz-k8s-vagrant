...
http://192.168.0.20:8081/#admin/repository/blobstores

Create blob store
  docker-hosted
  docker-hub

http://192.168.0.20:8081/#admin/repository/repositories
...
http://192.168.0.20:8081/#admin/security/realms
  add "Docker Bearer Token Realm" Active

docker login 192.168.0.20:5000
docker pull busybox
RMI=`docker images -a | grep busybox | awk '{print $3}'`
docker tag $RMI 192.168.0.20:5000/busybox:v20201225
docker push 192.168.0.20:5000/busybox:v20201225

http://192.168.0.20:8081/#browse/browse:docker-hosted
